...
from PyQt5.QtGui import QColor ,QPalette,QFont,QStandardItemModel,QStandardItem, QRegion
...
class FormToDelete(QWidget):
    def __init__(self):
        ...
        self.formLayout.addRow(QLabel("Классный руководитель:"), self.klassName)
...
class ListOfAbsence(QWidget):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("ClassLink QR for teacher")
        font = app.font()
    ...
    def load(self):
        ...
        students = cursor.execute('SELECT Surname, Name, Pat, InSchool, InClass FROM students ').fetchall()
        self.list.setColumnCount(5)
        self.list.setRowCount(len(students))
        self.list.setHorizontalHeaderLabels(['Фамилия','Имя','Отчество','В школе','В классе'])
...
class ListOfHomework(QWidget):
    ...
    def load(self):
        ...
        students = cursor.execute('SELECT Surname, Name, Pat, Homework FROM students ').fetchall()
        self.list.setColumnCount(4)
        self.list.setRowCount(len(students))
        self.list.setHorizontalHeaderLabels(['Фамилия','Имя','Отчество','Наличие домашки'])
        for s ,idxs in enumerate(students):
            for i, idxi in enumerate(idxs):
                if idxi == 0:
                    idxi = 'Нет'
...
class Main(QMainWindow):
    ...
    def __init__(self):
        ...
        list_of_NoHomework = self.menu_bar.addMenu("Домашке")
        ...
        self.showListHomework = QAction('Показать список наличия домашки',self)

        self.showListAbsence.triggered.connect(self.showAbsence)
        self.showListHomework.triggered.connect(self.showHomework)

        list_of_absence.addAction(self.showListAbsence)
        list_of_NoHomework.addAction(self.showListHomework)
    ...
    """))
        ...
        # radius = 20 
        # mask = QRegion(QRect(0, 0, self.width(), self.height()), QRegion.Ellipse)
        # self.student_info.setMask(mask)
    ...
    def display_student_info(self, current):
        if current:
            ...
            student_inf = cursor.execute(f'SELECT * FROM students WHERE Surname = "{surname}" AND Name = "{name}" AND Pat = "{pat}"').fetchall()[0] 
            print(student_inf)
            klass = student_inf[3]
            ...
            info += f'Фамилия: {surname}\nИмя: {name}\nОтчество: {pat}\nКласс: {klass}\nКлассный руководитель: {klassName}\nВ школе: {'Да' if inSchool == 1 else 'Нет'}\nВ классе: {'Да' if inKlass == 1 else 'Нет'}'
            
            self.QRLayout = QVBoxLayout(self)

            self.QR = QLabel(self)
            self.button = QPushButton('Создать PDF файл QR-кода')
            # png_pdf = fromPNGtoPDF.fromPNGtoPDF(f'{surname}_{name}_{pat}_{klass}.png')
            self.button.clicked.connect(self.createPDF)

            self.qr = QPixmap(f'{surname}_{name}_{pat}_{klass}.png')
            self.QR.setPixmap(self.qr.scaled(300, 300, aspectRatioMode=True))

        
            self.QRLayout.addWidget(self.QR)
            self.QRLayout.addWidget(self.button)
            self.layout.addLayout(self.QRLayout)
    ...
    def adding(self,data):
        ...
        try:
            print(f'INSERT INTO "students" (Surname, Name, Pat, Class,ClassName,Parent1,Parent2,InSchool,InClass,Homework) VALUES ("{data[0]}","{data[1]}","{data[2]}","{data[3]}","{data[4]}","{data[5]}","{data[6]}",0,0,"Не показал")')
            cursor.execute(f'INSERT INTO "students" (Surname, Name, Pat, Class,ClassName,Parent1,Parent2,InSchool,InClass,Homework) VALUES ("{data[0]}","{data[1]}","{data[2]}","{data[3]}","{data[4]}","{data[5]}","{data[6]}",0,0,"Не показал")')
    ...
    def deleting(self,data):
        ...
        try:
            ...
            if r == QMessageBox.Yes:
                cursor.execute(f'DELETE FROM students WHERE Surname = "{data[0]}" AND Name = "{data[1]}" AND Pat = "{data[2]}" AND Class = "{data[3]}" AND ClassName = "{data[4]}" AND Parent1 = "{data[5]}" AND Parent2 = "{data[6]}"')
            else:
                return
        except Exception as error:
            QMessageBox.critical(self, 'Ошибка',f'Ошибка: {error}')
    ...
    def auth(self,data):
        try:
            connection = sqlite3.connect('qrDatabase.db')
            cursor = connection.cursor()
            cursor.execute(f'SELECT * FROM teachers WHERE Name = "{data[0]}" AND Login = "{data[1]}" AND Password = "{data[2]}"')
            if cursor == []:
    ...
    def createPDF(self):
        try:
            ...
            print(self.pat)
            print(self.klass)
            print(f'{self.surname}_{self.name}_{self.pat}_{self.klass}.png')
            doc = aspose.pdf.Document()

            page = doc.pages.add()

            image = aspose.pdf.Image()
            image.file = f'{self.surname}_{self.name}_{self.pat}_{self.klass}.png'

            page.paragraphs.add(image)

            doc.save(f'{self.surname}_{self.name}_{self.pat}_{self.klass}..pdf')
        except Exception as error:
            QMessageBox.critical(self,'Ошибка',f'Ошибка: {error}')
        else:
            QMessageBox.information(self,'Файл создан','Файл создан: '+f'{self.surname}_{self.name}_{self.pat}_{self.klass}.pdf')
